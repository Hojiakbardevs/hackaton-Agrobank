...
import { AlertTriangle, TrendingUp, CreditCard, PhoneOff, PhoneCall, Shield, Brain, AlertCircle } from 'lucide-react';
    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-card border border-red-500/30 p-3 rounded-lg shadow-xl backdrop-blur-md">
                <p className="text-foreground font-bold text-sm">{payload[0].payload.name || payload[0].payload.year || payload[0].payload.type}</p>
                {payload.map((entry: any, index: number) => (
                    <p key={index} className="text-xs" style={{ color: entry.color }}>
                        {entry.name}: <span className="font-bold text-foreground">{entry.value}{entry.name === 'accuracy' ? '%' : ''}</span>
                    </p>
                ))}
            </div>
        );
...
export const Problem = () => {
    return (
        <section id="problem-solution" className="relative w-full py-24 overflow-hidden bg-background">
            <ProblemBackground />
                ...
                <div className="text-center mb-16 max-w-3xl mx-auto">
                    ...
                    >
                        ...
                        <p className="text-lg text-muted-foreground leading-relaxed">
                            O'zbekistonda kiberjinoyatchilik va telefon firibgarligi yildan-yilga keskin oshib bormoqda.
                            Oddiy himoya choralari endi yetarli emas.
                        </p>
                    </motion.div>
                </div>
                        ...
                        <div className="relative z-10 p-6">
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-4xl font-bold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">01</span>
                                </div>
                            ...
                            <div className="h-64 mb-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={fraudGrowthData}>
                                        <defs>
                                            <linearGradient id="colorCasesRed" x1="0" y1="0" x2="0" y2="1">
                                                <stop offset="5%" stopColor="#EF4444" stopOpacity={0.9} />
                                                <stop offset="95%" stopColor="#EF4444" stopOpacity={0.3} />
                                            </linearGradient>
                                        </defs>
                                        <CartesianGrid strokeDasharray="3 3" stroke="currentColor" className="text-border opacity-20" vertical={false} />
                                        <XAxis dataKey="year" stroke="currentColor" className="text-muted-foreground" fontSize={11} tickLine={false} axisLine={false} />
                                        <YAxis stroke="currentColor" className="text-muted-foreground" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(value) => `${value / 1000}k`} />
                                        <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(239, 68, 68, 0.1)' }} />
                                        <Bar dataKey="cases" fill="url(#colorCasesRed)" radius={[8, 8, 0, 0]} maxBarSize={40} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        ...
                        <div className="relative z-10 p-6">
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-4xl font-bold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">02</span>
                                </div>
                                <AlertCircle className="w-5 h-5 text-red-500" />
                            </div>
                            ...
                            <div className="h-64 mb-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={accuracyData}>
                                        <defs>
                                            <linearGradient id="colorAccuracyOrange" x1="0" y1="0" x2="0" y2="1">
                                        ...
                                        <YAxis stroke="currentColor" className="text-muted-foreground" fontSize={11} tickLine={false} axisLine={false} domain={[0, 100]} tickFormatter={(value) => `${value}%`} />
                        ...
                        <div className="relative z-10 p-6">
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-4xl font-bold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">03</span>
                                </div>
                                <AlertCircle className="w-5 h-5 text-red-500" />
                            </div>
                            ...
                            <div className="h-64 mb-4 flex items-center justify-center">
                                <ResponsiveContainer width="100%" height="100%">
                                    <RadarChart data={attackRadarData}>
                                        <PolarGrid stroke="currentColor" className="text-border opacity-30" />
                                        <PolarAngleAxis dataKey="type" stroke="currentColor" className="text-muted-foreground" fontSize={10} />
                                        <PolarRadiusAxis angle={90} domain={[0, 100]} stroke="currentColor" className="text-muted-foreground" fontSize={10} />
                                        <Radar name="Xavf darajasi" dataKey="threat" stroke="#EF4444" fill="#EF4444" fillOpacity={0.5} />
