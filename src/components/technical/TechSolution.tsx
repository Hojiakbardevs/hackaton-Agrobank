...
export function TechnicalSolution() {
  const flowSteps = [
    ...
    },
    {
      ...
      title: "Processing",
      description:
        "Raqam lokal bazada tekshiriladi, Whisper Tiny tahlil qiladi",
    ...
    <div className="w-full space-y-16">
      <motion.div
        ...
        <p className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
          On-device AI texnologiyasi orqali telefon firibgarligini real vaqtda
          aniqlash jarayoni
        </p>
      </motion.div>
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div
            className="w-full h-1 bg-linear-to-r from-transparent via-emerald-500/20 to-transparent hidden lg:block"
            style={{ top: "50%" }}></div>
        </div>
          ...
          {flowSteps.map((step, index) => (
            <motion.div
              ...
              <div className="relative bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 hover:border-emerald-500/50 transition-all duration-300 shadow-lg hover:shadow-emerald-500/20 cursor-pointer">
                {/* Gradient Background */}
                <div
                  className={`absolute inset-0 bg-linear-to-br ${step.color} opacity-5 rounded-2xl group-hover:opacity-10 transition-opacity duration-300`}></div>

                {/* Step Number */}
                <div
                  className={`absolute -top-3 -right-3 w-8 h-8 rounded-full bg-linear-to-r ${step.color} flex items-center justify-center text-white font-bold text-sm shadow-lg`}>
                  {index + 1}
                </div>
                ...
                <div className="relative z-10">
                  {/* Icon */}
                  <div
                    className={`w-16 h-16 rounded-2xl bg-linear-to-br ${step.color} flex items-center justify-center mb-4 mx-auto group-hover:scale-110 transition-transform duration-300`}>
        ...
        {[
          {
            ...
            description:
              "Qo'ng'iroq davomida aniqlash, hech qanday kechikishsiz",
          },
          {
            icon: <Shield className="w-6 h-6" />,
            title: "On-device Security",
            description:
              "Ma'lumotlar telefonda qoladi, internet talab qilmaydi",
          },
          {
